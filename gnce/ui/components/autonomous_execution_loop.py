# ui/components/autonomous_execution_loop.py
...
# -------------------------------------------------------
#  Minimal CSS Fix for White Text Issue
# -------------------------------------------------------
def _apply_minimal_css_fix():
    """Apply minimal CSS to fix white text in Platform Impact section only"""
    st.markdown("""
    <style>
    /* Minimal fix: Target ONLY the Platform Impact bullet points */
    div[data-testid="stVerticalBlock"]:has(h5:contains("🎯 Platform Impact")) + div .stMarkdown,
    div:has(h5:contains("🎯 Platform Impact")) ~ div .stMarkdown {
...
def _get_original_input_from_session() -> Dict[str, Any]:
    ...
    try:
        ...
        if "current_input_used" in st.session_state:
            current_input = st.session_state["current_input_used"]
            if current_input and isinstance(current_input, dict):
                return current_input
        
        # Fallback: try to get from input editor session state
        if "gn_input_json_text" in st.session_state:
            input_text = st.session_state["gn_input_json_text"]
            if input_text:
                try:
        ...
        # Fallback: get from latest ADRA
        if "current_adra" in st.session_state:
            latest_adra = st.session_state["current_adra"]
            if isinstance(latest_adra, dict):
                # Try to get from L2
                l2 = latest_adra.get("L2_input_snapshot_and_dra") or latest_adra.get("L2_input_snapshot_and_provenance") or latest_adra.get("L2")
        ...
        if is_minimal_payload:
            ...
            if "content" in payload and payload["content"]:
                ...
                if isinstance(content, str) and len(content) > 0:
                    st.markdown("**Content:**")
                    # Show content without background color or border
                    st.markdown(f"""
                    <div style="padding: 2px 0; margin: 4px 0;">
...
def render_autonomous_execution_loop(adra: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    # Apply minimal CSS fix for white text issue
    _apply_minimal_css_fix()
    ...
    with col_act:
        ...
        # Platform effects - The CSS fix will make this dark text
        st.markdown("---")
